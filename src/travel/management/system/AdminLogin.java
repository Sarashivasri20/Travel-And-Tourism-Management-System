...
public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == b1) {
            try {
                ...
                if (t1.getText().isEmpty() || passwordField.getPassword().length == 0) {
                ...
                st.setString(2, new String(passwordField.getPassword()));  // Convert char[] to String
    
                ResultSet rs = st.executeQuery();
                if (rs.next()) {
                    new AdminHome(t1.getText()).setVisible(true);  // Open admin home page
                    home.dispose();  // Close the current window (home)
                } else {
